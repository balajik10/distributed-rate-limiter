...
tmp_base="${TMPDIR:-/tmp}"
tmp_dir="$(mktemp -d "${tmp_base%/}/rate-limiter-failure-demo.XXXXXX")"
redis_stopped=0
...
curl_api --fail --silent --show-error "$base_url/api/v1/policies" >/dev/null
...
fail_open_status="$(curl_api --silent --show-error -o "$tmp_dir/fail-open.json" -w '%{http_code}' \
  -X POST "$base_url/api/v1/rate-limits/check" -H 'Content-Type: application/json' \
...
fail_closed_status="$(curl_api --silent --show-error -o "$tmp_dir/fail-closed.json" -w '%{http_code}' \
  -X POST "$base_url/api/v1/rate-limits/check" -H 'Content-Type: application/json' \
...
recovery_status="$(curl_api --silent --show-error -o "$tmp_dir/recovery.json" -w '%{http_code}' \
  -X POST "$base_url/api/v1/rate-limits/check" -H 'Content-Type: application/json' \
